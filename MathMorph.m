f=imread('\TestImageMorphological\d1.jpg'); % Reading the number plate image.
...
imwrite(gdiff, '\SegmentedImages\Morphedimage.jpg','jpg');
...
H=bwmorph(F,'thin',NaN); % Thinning the image to ensure character isolation.
...
for i = 1:length(Iprops)
   s = 'segment';
    ...
    s1 = strcat(s1,s);
   %figure('name', s)
    s = strcat('\SegmentedImages\',s,'.jpg');
